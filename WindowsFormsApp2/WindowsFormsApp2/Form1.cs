...
namespace WindowsFormsApp2
{
  ...
  {
    static double h_step = 0.05;
    /// <summary>
    /// Левая граница графика
    /// </summary>
    private double XMin = -0.7;
    /// <summary>
    /// Правая граница графика
    /// </summary>
    private double XMax = 1.5;
    private double z = 3.5 * Math.Pow(10, -2);
    private double t = 0.564849;
    /// <summary>
    /// Шаг графика
    /// </summary>
    private double Step = h_step;
    ...
    private void CalcFunction()
    {
      ...
      for (int i = 0; i < count; i++)
      {
        ...
        y1[i] = Math.Sin(Math.Sqrt((t)/((2*Math.Cos(x[i]-Math.PI/6))*(1+(Math.Pow(z,2))/(3-(Math.Pow(z,2)/2))))));
      //  y2[i] = Math.Cos(x[i]);
      }
    }
